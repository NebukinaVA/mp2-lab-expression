...
public:
	TExpression(string exp = "1+1");     // êîíñòðóêòîð ñ ïàðàìåòðîì
	~TExpression();
	string GetInfix() { return Infix; }
	string GetPostfix() { return Postfix; }
	void SetExpression(std::string exp); //ïîëó÷èòü âûðàæåíèå
	bool HasLetters(std::string exp);
	bool IsCorrect(std::string exp);     //ïðîâåðêà ïðàâèëüíîñòè
	void ToPostfix();                    //ïåðåâîä â ïîëüñêóþ çàïèñü
	double Calculate();                  // âû÷èñëåíèå
...
bool TExpression::HasLetters(std::string exp) // ïðîâåðèòü, åñòü ëè áóêâû
{
	for (int i = 0; i < exp.length(); i++)  
	{
		if (isalpha(exp[i]))
